<?php
	...
	$userpw = password_hash($_POST['wUserPW'], PASSWORD_DEFAULT);
	...
	{	
		...
		if(strlen($_POST['wUserPW']) >= 8){
			// $_POST로 받아온 두 비밀번호 비교후 일치하면 참
			if($_POST['wUserPW'] == $_POST['wUserPWConfirm']){
				//mq() 한수를 이용하여 쿼리를 $sql에 삽입
				$sql = mq("insert into member (id,pw,name) values('".$userid."','".$userpw."','".$email."')");
			?>
				<meta charset="utf-8" />
				<!-- alert() :  alert()은 경고창(alert box)를 나타내 주는 스크립트입니다.-->
				<script type="text/javascript">alert('회원가입이 완료되었습니다.');</script>
				<!-- 회원가입이 완료되었을 시 main.php로  -->
				<meta http-equiv="refresh" content="0 url=/register/main.php">
			<?php	
			} else {
			?>
				<meta charset="utf-8" />
		...
		?>
			...
			<meta http-equiv="refresh" content="00 url= /register//member.php">
		<?php 
		}
	} else {
	?>
