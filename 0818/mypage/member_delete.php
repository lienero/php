<?php
	...
	$sql = mq("select * from member where mem_id='".$_SESSION['userid']."'");
    ...
    if(isset($_SESSION['userid'])){
        if($_POST['userpw'] == $_POST['userpwconfirm']){
            if(password_verify($password, $hash_pw)) {
                if($agree == "agree"){
                    $sql = mq("delete from member where mem_id='".$_SESSION['userid']."'");
                    session_destroy();
                    echo "<script>alert('회원을 탈퇴하셨습니다.'); location.href='/back/index.php';</script>";
                } else {
